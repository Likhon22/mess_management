        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">Record and track member contributions for meals or house bills.</p>
                </div>
                <button
                    onClick={() => {
                        if (!isManager) {
                                ...
                                (t) => (
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <Link
                                            ...
                                            className="bg-white text-rose-600 px-3 py-1.5 rounded-lg text-xs font-black text-center border border-rose-100 hover:bg-rose-50 transition-colors uppercase"
                                        >
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-blue-100">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Payer (Who Paid?)</label>
                                <select
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <option value="house">House Payment (Rent + Bills)</option>
                                </select>
                ...
                <div className="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div className="space-y-3">
                        {pendingPayments.map((payment: Payment) => (
                            <div key={payment.id} className="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                <div>
                                    <p className="font-semibold text-gray-900">{formatCurrency(payment.amount)}</p>
                                    <p className="text-sm text-gray-600">Month: {payment.month}</p>
                                    <p className="text-xs text-gray-500 mt-1">Type: {payment.type === 'meal' ? 'Meal' : 'House'}</p>
                                </div>
                                <button
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                {myPayments.length > 0 ? (
                                ...
                                {myPayments.map((payment: Payment) => {
                                    ...
                                    return (
                                        <tr key={payment.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 font-medium text-gray-900">{payment.created_at ? new Date(payment.created_at).toLocaleDateString() : '-'}</td>
                                            <td className="px-6 py-4 text-gray-900 font-medium">
                                                {payer ? payer.user_name || payer.user_id : 'Unknown'}
                                            </td>
                                            <td className="px-6 py-4 font-bold text-gray-900">{formatCurrency(payment.amount)}</td>
                                            <td className="px-6 py-4 text-gray-900 capitalized font-medium">{payment.type}</td>
                                            <td className="px-6 py-4">
                                                <span className={`px-2 py-1 rounded-full text-xs font-bold ${payment.status === 'approved' ? 'bg-green-100 text-green-700' :
                    ...
                    <div className="p-6 text-center py-12">
                        ...
                        <p className="text-sm text-gray-400 mt-1">Payments recorded for you will appear here</p>
                    </div>
                )}
            </div>
        </div>
